...
def render_board_html(board, moves=None):
    ...
    """
    html += '<div class="board-container"><table>'
    for x in range(8):
        html += '<tr>'
        for y in range(8):
            ...
            content = ""
            if cell_value == 1:
                content = '<div class="stone" style="background:black;"></div>'
            elif cell_value == -1:
                content = '<div class="stone" style="background:white; border:1px solid black;"></div>'
            else:
                index = x*8+y
                content = f'<div class="index-number">{index}</div>'
            html += f'<td class="{classes}">{content}</td>'
        html += '</tr>'
    html += '</table></div>'
    return html

# --- Streamlit UI ---
st.set_page_config(layout="wide")  # ページ幅を広げる（スマホ対応）
